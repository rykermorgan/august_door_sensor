...
def augustFrontDoor():
    ...
    # returns enum
    frontDoor = api.get_lock_door_status(authentication.access_token,'BF4F854D9E9B4786A9FA3A8833AAE09A')

    # set string boolean for webhook 'state' value
    def frontDoorState():
        if str(frontDoor) == doorOpen:
            return 'false'
        elif str(frontDoor) == doorClosed:
            return 'true'

    # set current state variable for dynamic webhook 'state'
    currentState = frontDoorState()
    ...
    if previousState != currentState:
        ...
        d_homebridge = {'accessoryId': 'front-door', 'state': currentState}

        # Possible to set with public dataplicity hook for external use

        # ifttt_hook = 'https://maker.ifttt.com/trigger/front_door_state/with/key/dlkifzsgpVQlWG9Zus47mN'
        # d_ifttt = {'value1': 'front-door-2', 'value2': currentState}

        #postToWebhook

        homebridge_response = requests.post(url = homebridge_hook, params = d_homebridge)

        # ifttt_response = requests.post(url = ifttt_hook, params = d_ifttt)
